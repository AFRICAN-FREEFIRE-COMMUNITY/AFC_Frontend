...
export const env = createEnv({
  ...
  client: {
    ...
    // Cloudflare Turnstile site key (owner 2026-09-22): the PUBLIC half of the bot check on
    // signup, contact, feedback and the partner application. Optional on purpose: with no key
    // the widget is not rendered and the server allows the post, so a developer machine and the
    // scratch server keep working. The SECRET half lives only on the backend
    // (TURNSTILE_SECRET_KEY) and never appears in a NEXT_PUBLIC_ variable.
