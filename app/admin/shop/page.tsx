...
export default function AdminShopPage() {
  ...
  const handleAddItem = () => {
    if (newItem.name && newItem.amount > 0 && newItem.price > 0 && newItem.stock >= 0) {
  ...
  const handleUpdateItem = () => {
    if (editingItem.name && editingItem.amount > 0 && editingItem.price > 0 && editingItem.stock >= 0) {
      setShopItems(shopItems.map((item) => (item.id === editingItem.id ? editingItem : item)))
      setEditingItem(null)
      setImagePreview("")
  ...
  const handleDeleteItem = () => {
    setShopItems(shopItems.filter((item) => item.id !== itemToDelete.id))
              ...
              <TableBody>
                {shopItems.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <span
                        className={`px-2 py-1 rounded text-xs ${item.stock > 0 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                      >
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              ...
              <DialogDescription>Add a new product to your shop. Fill in all the details below.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="productAmount" className="text-right">
                  Diamonds
                </Label>
                <Input
                  id="productAmount"
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="sm:max-w-[500px]">
            ...
            {editingItem && (
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="editProductImage" className="text-right">
                    Image
                  </Label>
                  <div className="col-span-3">
                    <div className="flex items-center gap-4">
                ...
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label htmlFor="editProductDescription" className="text-right pt-2">
                    Description
                  </Label>
                  <Textarea
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="editProductAmount" className="text-right">
                    Diamonds
                  </Label>
                  <Input
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="editProductStock" className="text-right">
                    Stock
                  </Label>
                  <Input
                    id="editProductStock"
            ...
            {itemToDelete && (
              <div className="py-4">
                ...
                <p className="text-sm text-muted-foreground">
                  {itemToDelete.amount} Diamonds - ₦{itemToDelete.price.toLocaleString()}
                </p>
              </div>
            )}
            <DialogFooter>
