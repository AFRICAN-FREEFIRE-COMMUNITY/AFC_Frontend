...
// approved mockup tasks/home-additions-mockup.html:
...
// It is deliberately COMPACT (a slim status-accent strip instead of a big banner,
// tight cards, "View all" links) so it never adds a long scroll to the home page.
//
// Data sources (same endpoints the dedicated pages use, so the home preview can never
// diverge from them):
...
// Renders nothing (the whole block is hidden) until at least one source has data, so
// an empty community never shows empty section shells on the landing page.
...
export function HomeLatestSections() {
  ...
  // Nothing to show anywhere -> render nothing (no empty shells on the landing page).
  ...
  return (
    <div className="space-y-6 mb-4">
      {/* ── SECTION 1: Latest Tournaments & Scrims ── */}
      {events.length > 0 && (
        <section>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-3">
            <div>
              <h2 className="text-xl font-bold">Latest Tournaments &amp; Scrims</h2>
              <p className="text-sm text-muted-foreground">
                A quick look at the newest events. The full list lives on the events page.
              </p>
            </div>
            <Button asChild variant="outline" size="sm">
              <Link href="/tournaments">
                View all events <IconArrowRight className="ml-1 h-4 w-4" />
              </Link>
            </Button>
          </div>
      ...
      {/* ── SECTION 2: From the Player Market ── */}
      {allPosts.length > 0 && (
        <section>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-3">
            <div>
              <h2 className="text-xl font-bold">From the Player Market</h2>
              <p className="text-sm text-muted-foreground">
                The newest posts. Open the Player Market for everyone recruiting or available.
              </p>
            </div>
            <Button asChild variant="outline" size="sm">
              <Link href="/player-markets">
                Open Player Market <IconArrowRight className="ml-1 h-4 w-4" />
              </Link>
            </Button>
          </div>
