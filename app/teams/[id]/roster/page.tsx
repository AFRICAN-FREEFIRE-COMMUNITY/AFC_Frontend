...
export default function ManageRosterPage() {
  ...
  const handleRoleChange = (memberId: string, roleType: "inGameRole" | "managementRole", newRole: string) => {
    setTeamData((prevData) => {
      const updatedMembers = prevData.members.map((member) => {
        ...
        if (roleType === "managementRole" && (newRole === "Team Captain" || newRole === "Team Owner")) {
          return { ...member, managementRole: member.managementRole === newRole ? "None" : member.managementRole }
        }
        return member
      })
  ...
  const handleKickMember = async (memberId: string) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "An error occurred while removing the team member. Please try again.",
        variant: "destructive",
      })
    }
  }
                  ...
                  <TableRow key={member.id}>
                    ...
                    <TableCell>
                      <Select
                        ...
                        onValueChange={(value) => handleRoleChange(member.id, "inGameRole", value)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <TableCell>
                      <Select
                        ...
                        onValueChange={(value) => handleRoleChange(member.id, "managementRole", value)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <TableCell>
                      <Button variant="destructive" onClick={() => handleKickMember(member.id)}>
                        Kick
                      </Button>
                    </TableCell>
                  </TableRow>
