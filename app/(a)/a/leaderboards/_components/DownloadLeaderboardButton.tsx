...
//   nameX: YT_R_OFFSET + 105,
//   nameMaxW: 380,
//   booyahCx: YT_R_OFFSET + 540,
//   placeCx: YT_R_OFFSET + 640,
//   killsCx: YT_R_OFFSET + 730,
//   totalRx: YT_R_OFFSET + 1000,
// };
...
// YouTube: 9 rows per column (measured from template), 2 columns = 18 per image
const ROWS_PER_COL_YT = 9;
const ROWS_PER_PAGE_YT = ROWS_PER_COL_YT * 2;
...
async function drawTeamPageSocial(
  ...
  // 15 rows, header ends ~y=197, footer ~y=952
  const ROW_START_Y = 197;
  const ROW_H = 50.3; // (952 - 197) / 15
...
//
// Pixel-measured from AFC YT LEADERBOARD TEMPLATE.png:
//   Canvas:    1920 × 1080
//   Left box:  x=39  → x=907  (width 868)
//   Right box: x=1011 → x=1879 (width 868)
//   Right offset from left: 972px
//   Row height: 80px  (9 rows per column)
//   First row top: y=280  →  first row center y=320
//
// Column centers (left box, measured from header text):
//   #        cx=59
//   TEAM NAME nameX=85, maxW=320 (clears BOOYAH at 434)
//   BOOYAH   cx=434
//   PLACE    cx=522
//   KILLS    cx=592
//   TOTAL    right-align rx=895  (12px inside box right edge 907)
...
// Right column is identical layout, shifted 972px to the right
const YT_R_OFFSET = 972;
const YT_R = {
  rankCx: YT_R_OFFSET + 59, // 1031
  nameX: YT_R_OFFSET + 85, // 1057
  nameMaxW: 320,
  booyahCx: YT_R_OFFSET + 434, // 1406
  placeCx: YT_R_OFFSET + 522, // 1494
  killsCx: YT_R_OFFSET + 592, // 1564
  totalRx: YT_R_OFFSET + 895, // 1867
};

async function drawTeamPageYouTube(
  rows: any[], // up to 18 rows (9 left + 9 right)
  startRank: number,
  pageNum: number,
  totalPages: number,
  killPoint: number,
  ...
  // Pixel-measured: first box top = y=280, row height = 80px
  // Row centers: 320, 400, 480, 560, 640, 720, 800, 880, 960
  const ROW_START_Y = 263;
  const ROW_H = 80;
...
async function drawPlayerPageYouTube(
  ...
  // Reuse the same x-positions, remapping columns to player stats:
  // # | USERNAME | TEAM | KILLS | DAMAGE | ASSISTS
  const leftCols = {
    rankCx: YT_L.rankCx,
    nameX: YT_L.nameX,
