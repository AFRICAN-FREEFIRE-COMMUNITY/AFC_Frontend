...
// support it). Copy is internationalized under the ocr.uploadSteps.logUpload namespace (admin now in
// i18n scope, owner 2026-07-13); English is authored here, fr/pt generated by pnpm i18n:translate.
//
// COMPONENTS:
...
export function MultiMapLogPanel({
  ...
  // Per-file "Match N (map)" label. Lives inside the component so it can translate.
  const matchLabel = (m: MatchOption) =>
    m.match_map
      ? t("uploadSteps.logUpload.matchLabelWithMap", {
          n: m.match_number,
          map: m.match_map,
        })
      : t("uploadSteps.logUpload.matchLabel", { n: m.match_number });
  ...
  const validate = (): string | null => {
    if (rows.length === 0) return t("uploadSteps.logUpload.addAtLeastOne");
    if (rows.some((r) => !r.matchId))
      return t("uploadSteps.logUpload.assignMatch");
    const ids = rows.map((r) => r.matchId);
    if (new Set(ids).size !== ids.length)
      return t("uploadSteps.logUpload.duplicateMatch");
    return null;
  };
  ...
  const runPass = async (dryRun: boolean) => {
          ...
          rs.map((r, j) =>
            j === i
              ? {
                  ...r,
                  status: "error",
                  error: e?.message || t("uploadSteps.logUpload.uploadFailed"),
                }
              : r,
          ),
        );
    ...
    if (!dryRun && ok > 0) {
      // Pick the "all applied" vs "some failed" message in JS so the ICU strings stay plain plurals
      // (the codebase convention), no empty-branch nesting.
      toast.success(
        failed === 0
          ? t("uploadSteps.logUpload.appliedAll", { ok })
          : t("uploadSteps.logUpload.appliedSome", { ok, failed }),
      );
      onChanged?.();
      if (failed === 0) {
        setRows([]);
      ...
      toast.success(
        failed === 0
          ? t("uploadSteps.logUpload.reviewedAll", { ok, total: rows.length })
          : t("uploadSteps.logUpload.reviewedSome", {
              ok,
              total: rows.length,
              failed,
            }),
    ...
    <div className="flex flex-col">
      <div className="flex-1 space-y-3">
        ...
        >
          ...
          <span className="text-sm font-medium">
            {t("uploadSteps.logUpload.dropzoneTitle")}
          </span>
          <span className="text-xs">
            {t("uploadSteps.logUpload.dropzoneHint")}
          </span>
        </div>
        <input
          ref={fileRef}
          ...
          <div key={row.id} className="rounded-lg border p-3 text-xs">
            <div className="flex items-center gap-2">
              ...
              {!busy && (
                <button
                  ...
                  aria-label={t("uploadSteps.logUpload.removeFile")}
                >
                  <IconX size={14} />
                </button>
              )}
            ...
            <div className="mt-2 flex items-center gap-2">
              <span className="text-muted-foreground">
                {t("uploadSteps.logUpload.mapMatch")}
              </span>
              <Select
                value={row.matchId}
              ...
              >
                <SelectTrigger className="h-8 w-[220px] text-xs">
                  <SelectValue
                    placeholder={t("uploadSteps.logUpload.selectMatch")}
                  />
                </SelectTrigger>
                <SelectContent>
                  {validMatches.map((m) => (
            ...
            {row.preview && row.status !== "error" && (
              <div className="mt-2 flex flex-wrap gap-1.5">
                <Badge variant="outline" className="rounded-full">
                  {isSolo
                    ? t("uploadSteps.logUpload.parsedRows", {
                        count: row.preview.parsed,
                      })
                    : t("uploadSteps.logUpload.parsedTeams", {
                        count: row.preview.parsed,
                      })}
                </Badge>
                <Badge variant="outline" className="rounded-full border-primary text-primary">
                  {isSolo
                    ...
                    : t("uploadSteps.logUpload.savedTeam", {
                        count: row.preview.saved,
                      })}
                </Badge>
                  ...
                  >
                    {isSolo
                      ? t("uploadSteps.logUpload.unmatchedSolo", {
                          count: row.preview.unmatched,
                        })
                      : t("uploadSteps.logUpload.unmatchedTeam", {
                          count: row.preview.unmatched,
                        })}
                  </Badge>
                )}
                ...
                <div className="mt-2 rounded-md border border-amber-500/40 bg-amber-500/5 p-2">
                  <p className="flex items-center gap-1 text-[11px] text-amber-600">
                    ...
                    {t("uploadSteps.logUpload.teamsNotFound", {
                      count: row.preview.missingTeams.length,
                      list: row.preview.missingTeams.join(", "),
                    })}
                  </p>
                </div>
              )}
        ...
        {rows.length === 0 && (
          <p className="py-2 text-center text-xs text-muted-foreground">
            {t("uploadSteps.logUpload.noFilesYet")}
          </p>
        )}
        {validMatches.length === 0 && (
          <p className="text-center text-xs text-amber-600">
            {t("uploadSteps.logUpload.noSlots")}
          </p>
        )}
      </div>
      ...
      <div className="mt-3 flex items-center justify-between gap-2 border-t pt-3">
        <span className="text-xs text-muted-foreground">
          {t("uploadSteps.logUpload.fileCount", { count: rows.length })}
        </span>
        <div className="flex gap-2">
          ...
          >
            {busy ? (
              ...
              t("uploadSteps.logUpload.review")
            )}
          </Button>
          <Button
            onClick={() => runPass(false)}
            disabled={busy || rows.length === 0 || !canManage || validMatches.length === 0}
            title={allReviewed ? "" : t("uploadSteps.logUpload.reviewFirstTip")}
          >
            {busy ? (
              <span className="flex items-center gap-2">
                <IconLoader2 size={16} className="animate-spin" />{" "}
                {t("uploadSteps.logUpload.applying")}
              </span>
            ) : (
              <>
                <IconCircleCheck size={16} /> {t("uploadSteps.logUpload.applyAll")}
              </>
            )}
          </Button>
        </div>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" disabled={!canManage}>
          <IconStack2 size={16} /> {t("uploadSteps.logUpload.trigger")}
        </Button>
      </DialogTrigger>
      <DialogContent className="flex max-h-[85vh] flex-col sm:max-w-[640px]">
        <DialogHeader>
          <DialogTitle>{t("uploadSteps.logUpload.dialogTitle")}</DialogTitle>
          <DialogDescription>
            {t("uploadSteps.logUpload.dialogDescription")}
          </DialogDescription>
        </DialogHeader>
...
function RowStatusBadge({ status }: { status: RowStatus }) {
  ...
  const map: Record<RowStatus, { label: string; cls: string }> = {
    pending: {
      label: t("uploadSteps.logUpload.status.ready"),
      cls: "text-muted-foreground",
    },
    reviewing: {
      label: t("uploadSteps.logUpload.status.reviewing"),
      cls: "text-muted-foreground",
    },
    reviewed: {
