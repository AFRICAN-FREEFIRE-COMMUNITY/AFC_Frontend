...
// per team, evenly across the groups, and shows the seal), OPENS it with a close time (every
// captain is notified and turns a card over on the event page), and either lets it CLOSE by itself
// at the deadline or closes it now (whoever has not picked is dealt into what is left). RESET
// throws the draw away while no result exists in the stage.
//
// WHAT IT RENDERS
//   1. stage picker (stages with groups)
//   2. the state of that stage's draw: none / draft / open / closed, with the controls that make
//      sense in that state and a live mini-board (numbered cards, who took what)
//   3. the seal: the commitment while open, plus the salt once closed, so the organizer can point
//      players at it
//
// HOW IT CONNECTS
//   - Data: lib/draws.ts drawsApi -> afc_draws/views.py (create / open / close / reset / board).
...
import { Dices, RefreshCw, Trash2 } from "lucide-react";
...
// The day the card went live (NEW badge, 5 days, self-expiring).
...
// datetime-local wants "YYYY-MM-DDTHH:MM" in the browser's own zone. Default the close time to
// 24 hours from now, rounded to the next hour, which is what most organizers want anyway.
...
export function GroupDrawCard({ eventId, stages, onRefresh }: Props) {
  ...
  const [busy, setBusy] = useState<"create" | "open" | "close" | "reset" | null>(null);
  ...
  const handleCreate = () => run("create", () => drawsApi.create(Number(stageId)), "draw.toastCreated");
  const handleOpen = () => {
    if (!board) return;
    ...
    run("open", () => drawsApi.open(board.draw_id, when.toISOString()), "draw.toastOpened");
  };
  const handleClose = () => {
    if (!board) return;
    if (!window.confirm(t("draw.confirmClose", { count: board.cards_total - board.cards_taken }))) return;
    run("close", () => drawsApi.close(board.draw_id), "draw.toastClosed");
  };
  const handleReset = () => {
    if (!board) return;
    if (!window.confirm(t("draw.confirmReset"))) return;
    run("reset", () => drawsApi.reset(board.draw_id), "draw.toastReset");
  };
        ...
        {board && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {board.status === "open" && (
                <Button size="sm" variant="outline" onClick={handleClose} disabled={busy !== null}>
