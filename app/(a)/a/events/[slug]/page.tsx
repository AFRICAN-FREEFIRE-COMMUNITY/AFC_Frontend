...
const Page = ({ params }: { params: Promise<Params> }) => {
  ...
  const generateBulkInvites = async () => {
    ...
    try {
      ...
      toast.success(`${count} invite links generated successfully!`);
      ...
      toast.error(
        error?.response?.data?.message || "Failed to generate invite links",
      );
    } finally {
      setGeneratingInvite(false);
    }
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
        <div className="w-full">
          ...
          <div className="flex gap-2 overflow-x-auto whitespace-nowrap p-1 custom-scroll">
            ...
            <Badge
              className={is_public ? "hidden" : ""}
              variant={is_public ? "default" : "destructive"}
            >
              {is_public === false && (
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>This is a private event</TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              )}
            </Badge>
