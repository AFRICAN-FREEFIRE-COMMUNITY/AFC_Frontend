...
// //                             // This prevents the "12" issue when the user wants to type "2" over "1".
// //                             value={
...
// //                               // Allow empty string to clear the field (which will fail zod validation, prompting user)
// //                               if (val === "") {
// //                                 field.onChange("");
// //                               } else {
// //                                 // Pass the raw string to react-hook-form's change handler.
// //                                 // Zod's coerce handles the conversion on submission/trigger.
// //                                 field.onChange(val);
// //                               }
// //                             }}
// //                             placeholder="e.g., 128"
...
// //                             type="number"
// //                             min={1}
// //                             // The key fix is setting the value to string or empty string
// //                             value={
// //                               field.value === undefined || field.value === null
// //                                 ? ""
// //                                 : field.value.toString()
...
// //                               // Call the custom handler only if a numeric value is present
// //                               if (val !== "") {
// //                                 handleStageCountChange(Number(val));
// //                               } else {
// //                                 // Handle case when input is cleared by passing 0 or default value
...
// //                           <Input
// //                             type="number"
// //                             value={value === 0 ? "" : value} // If value is 0, display empty string for easier typing
// //                             onChange={(e) => {
// //                               const inputVal = e.target.value;
// //                               const updated = { ...prizeDistribution };

// //                               // Use a simple Number() conversion here for display purposes.
// //                               // If the input is empty or invalid, Number(inputVal) will be 0 or NaN.
// //                               // We will store the actual number, but allow the user to type an empty field.
// //                               updated[key] =
// //                                 inputVal === "" ? 0 : Number(inputVal); // Set to 0 if cleared
...
// //                       type="number"
// //                       min={0}
// //                       // Convert number to string for input value, or "" if 0/undefined/null
// //                       value={
...
//   playing_date: z.string().min(1, "Playing date required"),
//   playing_time: z.string().min(1, "Playing time required"),
//   // Keep coercion, ensure we handle non-numeric input for better UX
//   teams_qualifying: z.coerce.number().min(1, "Must qualify at least 1 team"),
// });
...
//   start_date: z.string().min(1, "Start date required"),
//   end_date: z.string().min(1, "End date required"),
//   // Keep coercion, handle input via string value
//   number_of_groups: z.coerce.number().min(1, "Must have at least 1 group"),
//   stage_format: z.string().min(1, "Stage format required"),
//   groups: z.array(GroupSchema).min(1, "At least one group required"),
//   // Keep coercion, handle input via string value
...
//   number_of_stages: z.coerce.number().min(1, "At least 1 stage required"),
//   stages: z.array(StageSchema).min(1, "At least one stage required"),
//   prizepool: z.string().min(1, "Prize pool required"), // Assuming prizepool is currency/string input
//   // Keep coercion for distribution values
//   prize_distribution: z.record(z.string(), z.coerce.number()),
//   event_rules: z.string().optional(),
//   rules_document: z.any().optional(),
//   start_date: z.string().min(1, "Start date required"),
...
//   // Assuming useAuth is correctly imported
//   const { user, token } = useAuth();
...
//       participant_type: "",
//       event_type: "",
//       // Use a number default, but remember the input field handles it as a string
//       max_teams_or_players: 1,
//       banner: "",
//       stream_channels: [""],
//       event_mode: "",
...
//         // We already validate the groups minimum count and required fields in handleSaveStage
//         const allStagesValid = form
//           .getValues("stages")
//           .every((stage) => stage.groups && stage.groups.length > 0);
//         if (!allStagesValid) {
...
//                 </CardHeader>
//                 <CardContent className="space-y-4">
//                   {/* FIX: number_of_stages controlled component for number input */}
//                   <FormField
//                     control={form.control}
//                     name="number_of_stages"
//                     render={({ field }) => (
...
//                         <FormControl>
//                           <Input
//                             type="number" // NOTE: Changed to text to allow custom formatting/currency symbols if needed, but keeping field coercion on submit.
...
//                             onChange={(e) => {
//                               const val = e.target.value;
//                               // Pass raw string to RHF for coercion on blur/submit
//                               field.onChange(val);
//                             }}
//                             placeholder="e.g., 5000"
//                             className=""
...
//                         <Input value={key} disabled className="col-span-1" />
//                         <div className="col-span-3 flex items-center justify-end gap-1">
//                           {/* FIX: Prize distribution input controlled component for number input */}
//                           <Input
//                             type="number"
...
//               <div className="ml-auto flex gap-3">
//                 {currentStep < 6 ? (
//                   // If we are on Step 1 through 5, click Next and validate fields
//                   <Button
//                     type="button"
//                     onClick={handleNextStep}
//                     disabled={isPending}
...
//                   </Button>
//                 ) : currentStep === 6 ? (
//                   // If we are on Step 6, click Next to validate rules and advance to Step 7 (Review)
//                   <Button
//                     type="button"
//                     onClick={handleNextStep}
//                     disabled={isPending}
...
//                   </Button>
//                 ) : (
//                   // If we are on Step 7, show the final submit button
//                   <Button type="submit" disabled={isPending}>
//                     {isPending ? "Creating..." : "Create Event"}
//                   </Button>
//                 )}
