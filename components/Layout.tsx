    ...
    <Sheet>
      ...
      <SheetContent className="w-full sm:max-w-md overflow-y-auto">
        ...
        <div className="py-6 space-y-6">
          {cartItems.map((item) => (
            <div key={item.id} className="flex flex-col space-y-2 border-b pb-4">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-medium">{item.name}</h3>
                  <p className="text-sm text-muted-foreground">₦{item.price.toLocaleString()}</p>
                  {item.isGift && <p className="text-xs text-blue-500">Gift to: {item.giftEmail}</p>}
                </div>
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const checkLoginStatus = async () => {
      ...
      if (!["/", "/login", "/create-account"].includes(pathname)) {
        login()
      }
    }
    checkLoginStatus()
        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-4">
            <Link href={isLoggedIn ? "/home" : "/"} className="flex items-center space-x-2">
              ...
              <span className="text-xl font-bold gold-text">AFC DATABASE</span>
            </Link>
          </div>
          <div className="flex items-center space-x-4">
            {isLoggedIn && (
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="w-[300px] sm:w-[400px]">
                <SheetHeader>
                  ...
                  <SheetDescription>Navigation menu for AFC DATABASE</SheetDescription>
                </SheetHeader>
                <SidebarContent isLoggedIn={isLoggedIn} onLogin={login} onLogout={handleLogout} userRole={userRole} />
              </SheetContent>
            </Sheet>
          </div>
        </div>
