...
export function Notifications() {
  ...
  const handleTeamInvite = (notificationId, accept) => {
    // Here you would make an API call to accept or deny the team invite
    console.log(`Team invite ${accept ? "accepted" : "denied"} for notification ${notificationId}`)
                ...
                <div className="py-2 px-1">
                  ...
                  {notification.type === "team_invite" && (
                    <div className="flex justify-end space-x-2 mt-2">
                      <Button size="sm" variant="outline" onClick={() => handleTeamInvite(notification.id, false)}>
                        Deny
                      </Button>
                      <Button size="sm" onClick={() => handleTeamInvite(notification.id, true)}>
                        Accept
                      </Button>
                    </div>
                  )}
